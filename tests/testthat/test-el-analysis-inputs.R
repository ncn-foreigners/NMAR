test_that("el_prepare_analysis_context validates mask and weights", {
  ...
  expect_error(
    NMAR:::el_prepare_analysis_context(df, "Y_miss", mask[-1]),
    "mask must have the same length",
    fixed = FALSE
  )

  expect_error(
    NMAR:::el_prepare_analysis_context(df, "Y_miss", mask, weights_full = rep(1, 5)),
    "weights_full",
    fixed = FALSE
  )

  res <- NMAR:::el_prepare_analysis_context(df, "Y_miss", mask, variance_method = "none")
