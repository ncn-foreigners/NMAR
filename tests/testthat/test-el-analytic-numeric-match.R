...
test_that("analytic vs numeric gradient of g agree (untrimmed)", {
  ...
  sol <- nleqslv::nleqslv(x = c(rep(0, ncol(Z)), 0), fn = eq_fun, jac = jac_fun, method = "Newton")
...
# Analytic gradient
  g_anal <- NMAR:::el_grad_g_analytic(fam, Z, Xc, mu_x, wts, eta, w_i, W, denom, lambda_W, resp_df$Y_miss, sum(wts), N_pop)
# Numeric gradient via mean function builder
  g_fn <- NMAR:::el_build_mean_fn(fam, Z, Xc, mu_x, wts, N_pop, sum(wts), Inf, resp_df$Y_miss)
