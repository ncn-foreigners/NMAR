...
test_that("el_prepare_design expands dot notation via Formula", {
  ...
  design <- NMAR:::el_prepare_design(Y_miss ~ . | ., df)
...
test_that("el_prepare_design accepts transformed outcomes and tracks source column", {
  ...
  design <- NMAR:::el_prepare_design(log(Y_miss) ~ X, df)
...
test_that("LHS expressions referencing multiple outcome variables are rejected", {
  ...
  expect_error(
    NMAR:::el_prepare_design(I(Y_miss + Y2) ~ X, df),
...
test_that("transforms that introduce NA for respondents are rejected", {
  ...
  expect_error(
    NMAR:::el_prepare_design(log(Y_miss) ~ X, df),
...
test_that("response intercept is retained even when formula uses +0", {
  ...
  expect_warning(
    design <- NMAR:::el_prepare_design(Y_miss ~ 1 | Z + 0, df),
...
test_that("explicit intercept plus auxiliaries triggers a warning", {
  ...
  expect_warning(
    design <- NMAR:::el_prepare_design(Y_miss ~ 1 + X, df),
    "Auxiliary intercepts are ignored",
    fixed = FALSE
  )
...
# '-1' should remove any implicit intercept silently
  design_m1 <- NMAR:::el_prepare_design(Y_miss ~ X - 1 | Z, df)
...
# '+0' should behave identically and silently
  design_p0 <- NMAR:::el_prepare_design(Y_miss ~ X + 0 | Z, df)
...
test_that("el_prepare_design rejects formulas with more than two RHS partitions", {
  ...
  expect_error(
    NMAR:::el_prepare_design(Formula::Formula(Y_miss ~ X1 | X2 | X3), df),
...
test_that("auxiliary constraints cannot reference the outcome", {
  ...
  expect_error(
    NMAR:::el_prepare_design(Y_miss ~ Y_miss + X, df),
...
test_that("constant auxiliary columns created via transforms are rejected", {
  ...
  expect_error(
    NMAR:::el_prepare_design(Y_miss ~ I(0 * X + 1) + X, df),
...
test_that("el_prepare_design handles language objects coerced to formula", {
  ...
  parsed <- NMAR:::el_prepare_design(lang_formula, df)
...
test_that("response-only formulas drop RHS2 entirely", {
  ...
  parsed <- NMAR:::el_prepare_design(Y_miss ~ X, df)
...
test_that("offset() usage is rejected on auxiliary and response partitions", {
  ...
  expect_error(
    NMAR:::el_prepare_design(Y_miss ~ offset(X) + Z, df),
    "Offsets .* auxiliary predictors",
    fixed = FALSE
  )
  expect_error(
    NMAR:::el_prepare_design(Y_miss ~ Z | offset(X), df),
...
test_that("auxiliary predictors with zero variance among respondents trigger an error", {
  ...
  expect_error(
    NMAR:::el_prepare_design(Y_miss ~ A, df),
    regexp = "Auxiliary covariate .* zero variance",
    fixed = FALSE
  )
})
...
test_that("missingness predictors with zero variance among respondents only warn", {
  ...
  expect_warning(
    design <- NMAR:::el_prepare_design(Y_miss ~ X | Z, df),
