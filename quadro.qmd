...
- `exptilt_engine()` — Exponential Tilting (Kim and Riddles 2012) 
- `exptilt_nonparam_engine()` — Nonparametric Exponential Tilting (Riddles et al. 2016)
...
## Empirical Likelihood: Core Idea

**Based on Qin, Leung & Shao (JASA, 2002).**  
Model only the **response mechanism**; keep the outcome distribution **nonparametric**.

> $$
> w(y,x;\beta)=P(\delta=1 \mid Y{=}y,X{=}x)\quad(\text{e.g., logit/probit}),\qquad
...
<div class="eq-card">
> $$
> p_i \;=\; \frac{1}{n}\,\frac{1}{\,1+\lambda_W\big(w(y_i,x_i;\beta)-W\big)+(x_i-\mu_x)^\top\lambda_x\,}.
> $$
...
## Empirical Likelihood: Mean Estimate

**Estimator of the mean**
<div class="eq-card">
> $$
> \hat\mu_Y \;=\; \sum_{i:\, \delta_i=1} p_i\,y_i
> $$
...
Extends to complex surveys by incorporating design weights in the estimating equations.
