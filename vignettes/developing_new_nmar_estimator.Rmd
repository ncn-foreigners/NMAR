...
>   `source("build_r_folder.R")` (or `Rscript build_r_folder.R`).
...
```{r eval=FALSE}
method_engine <- function(standardize = TRUE,
...
method.data.frame <- function(data, formula, standardize = TRUE, auxiliary_means = NULL, ...) {
  ...
  mu_x <- auxiliary_means

  # Shared scaling
  sc <- validate_and_apply_nmar_scaling(
    standardize = standardize, has_aux = !is.null(X_un) && ncol(X_un) > 0,
...
- The parent S3 class `nmar_result` provides default methods (`estimate`, `vcov`, `confint`, `tidy`, `glance`, `coef`, `fitted`, `weights`, `formula`). Implement child print/summary only if you need extra presentation.
...
- Lists:
  ...
  - `weights_info`: list with
    - `values`: unnormalised analysis weights or masses on the engine’s scale,
