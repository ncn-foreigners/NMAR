...
### Engines
- Family: "logit" (default) or "probit". Conceptually the score with respect to the linear predictor is
  $s_i = \partial\log w_i/\partial\eta_i = \mu_{\eta,i}/w_i$. In the implementation we use numerically stable
  formulas: for logit, $s_i = 1 - w_i$; for probit, $s_i = \phi(\eta_i)/\Phi(\eta_i)$ computed in the log domain.
...
which reduces exactly to the original QLS formula when $N_{\text{pop}} = n$. This closed form is used
...
Normalizing to enforce $\sum_i a_i p_i = 1$ (which reduces to $\sum p_i = 1$ when $a_i \equiv 1$) yields $p_i = \frac{D_i^{-1}}{\sum_j D_j^{-1}}$. In the presence of base sampling weights $a_i$ (survey designs), the same derivation gives the natural generalization
...
- **standardize** (default: TRUE)
  - Standardize $Z$/$X$ (and $\mu_x$) using a `nmar_scaling_recipe` for numerical stability, using base
    weights when present. Coefficients (and any available vcov) are unscaled after solving.
