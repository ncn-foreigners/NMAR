...
Mapping to code:
- `response_model_matrix` corresponds to $Z$ (includes an intercept column)
...
### Survey-Weighted Generalization of $\lambda_W$ (QLS Eq. 10)

QLS derive $\lambda_W = (N/n - 1)/(1-W)$ when $a_i \equiv 1$ and $n$ counts respondents. In survey settings, let $A_{\text{resp}} = \sum_{i:R_i=1} a_i$ be the respondent-weighted total and let $N_{\text{pop}}$ be the analysis-scale population total (either $\sum a_i$ or a user-supplied $n_{\text{total}}$). We use the weighted analogue

$$\lambda_W = \frac{N_{\text{pop}}/A_{\text{resp}} - 1}{1 - W},$$

which reduces to QLS when $a_i \equiv 1$.
...
Introducing Lagrange multipliers $(\lambda_0,\lambda_x,\lambda_W)$ for these constraints and profiling the $p_i$'s gives the KKT stationarity conditions (in a survey-weighted setting, with base weights $a_i$ acting as multiplicities in the empirical likelihood)

\[
\frac{\partial}{\partial p_i} \Big[ \sum_j a_j\, \log p_j - \lambda_0 (\sum_j p_j - 1) - \lambda_x^T \sum_j p_j (X_j - \mu_x) - \lambda_W \sum_j p_j (w_j - W) \Big] = 0,
\]
...
This is exactly the working form used in our estimator. With survey base weights, the working masses are proportional to $a_i/D_i$, so $p_i^{\text{EL}} \propto a_i/D_i$. The EL weights $p_i^{\text{EL}}$ are then used to build the mean estimator
...
n_{\text{resp\_weighted}} = \sum_i a_i,\qquad N_{\text{pop}} = n,
\]
QLS show that
\[
...
\]
for each nonreference stratum $h$. This forces the EL weights to reproduce the design-implied stratum shares while still adjusting within strata for NMAR. In the implementation this augmentation is performed in `el_run_core_analysis()` (see `src_dev/engines/el/impl/inputs.R`), and the resulting augmented auxiliaries flow through to `el_build_equation_system()` or `el_build_equation_system_survey()` depending on the data type.
...
## Analytical Jacobian ($A$ Matrix, IID path)
...
Analytical delta variance for the EL estimator has not been implemented in the
package. When requested via `variance_method = "delta"`, the implementation
returns `NA` with a guidance message. We recommend `variance_method = "bootstrap"`
for standard errors in both IID and survey settings.

Implementation: The analytical delta variance for EL is disabled in the codebase.
...
For standard errors, use bootstrap variance. Analytical delta variance for EL is
disabled in this version; derivations are omitted.
...
- **Estimating equations with base weights:** All sums include the base weight $a_i$; set $a_i$ to the survey design weight for respondents. Totals $N_{\text{pop}}$ and $n_{\text{resp\_weighted}}=\sum a_i$ are computed from the design weights, which feeds into $\lambda_W = ((N_{\text{pop}}/n_{\text{resp\_weighted}}) - 1)/(1-W)$.
...
This matches the paper's guidance to adapt the likelihood/estimating framework to stratification or unequal-probability sampling while relying on standard survey resampling for uncertainty. Analytical delta variance is not implemented for EL at this time.
...
- If $A$ appears ill-conditioned: prefer `variance_method = "bootstrap"` and report diagnostics (e.g., `jacobian_condition_number`). The package does not apply ridge or pseudoinverse at runtime.
