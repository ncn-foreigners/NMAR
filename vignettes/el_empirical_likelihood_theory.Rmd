...
description: >
  Mathematical details behind the empirical likelihood estimator for NMAR data (Qin, Leung, and Shao 2002) DOI:
...
- **Estimator functional** $g(\theta) = \hat{Y}(\theta)$: recompute $p_i^{\text{EL}}(\theta)$ from $\theta$ via $D_i(\theta)$, then $\hat{Y} = \sum p_i^{\text{EL}} Y_i / \sum p_i^{\text{EL}}$.
- **Gradient**:
  - When there is no trimming ($\text{cap} = \infty$), we use a closed‑form analytic gradient of the ratio $\hat Y(\theta)$, with $\partial \pi_i/\partial \theta$ derived under the same denominator guard $D_i \ge \varepsilon$ used in the estimating equations. This yields a smooth, numerically stable $\nabla g(\theta)$.
  - When trimming is active (non‑smooth map), we use a stabilized numeric gradient with finite‑difference settings tuned for this problem.
- **Variance**: $\mathrm{Var}(\hat{Y}) \approx \nabla g \, A^{-1} B (A^{-1})^T \nabla g^T$ using the (totals‑scale) $A$ and $B$ as defined above.

Practical note on stability: when the unconditional response rate $W$ is low and auxiliaries are weak, $\lambda_W$ can be large and some denominators $D_i$ may approach zero, creating very large EL weights and a heavy‑tailed sampling distribution for $\hat Y$. In such regimes a first‑order (delta) approximation may understate across‑sample variability. Prefer `variance_method = "bootstrap"` or strengthen auxiliary constraints. With trimming, the estimator is non‑smooth, so bootstrap is recommended for inference.
...
- This reparameterization and initialization improve stability in strong NMAR settings. See `src_dev/engines/el/impl/core.R`.
...
- `jacobian_condition_number` ($\kappa(A)$), `jacobian_source` (analytic vs numeric), `jacobian_auto_rule` (why numeric was chosen), `max_equation_residual`, and the trimming fraction.
- For variance, whether ridge or pseudoinverse was used (`used_ridge`, `used_pseudoinverse`, `invert_rule`).
...
Remark on calibration under complex designs: the analytic delta variance is derived from a first‑order expansion of the score‑total map. Under strong weighting or complex designs, the linearization can under‑estimate variability even though the form of $\widehat B$ is correct. In such cases, prefer bootstrap variance via replicate weights (`svrep`) for more reliable calibration.

### Trimming and Delta Approximation

When a finite trimming cap is used (non‑smooth map in $\theta$), the delta method is only approximate because $g(\theta)$ includes thresholding. The implementation therefore:

- Uses a closed‑form analytic $\nabla g$ only in the smooth, untrimmed case ($\text{cap}=\infty$).
- Falls back to a stabilized numeric gradient when trimming is active.

In practice, for trimmed fits or ill‑conditioned Jacobians, prefer `variance_method = "bootstrap"` for reliable uncertainty quantification.
...
- Engine: `el_engine(..., family, standardize, trim_cap, variance_method, ...)` in `src_dev/engines/el/engine.R`
  ...
  - Family objects: `logit_family()`, `probit_family()` in `src_dev/shared/families.R`.
...
- EL Core: `el_estimator_core(...)` in `src_dev/engines/el/impl/core.R` runs:
  ...
  - Build equations via `el_build_equation_system(family, ...)`
  - Solve with `nleqslv` (multi-start fallback and Broyden if needed)
  - Compute EL weights, Y_hat, diagnostics, constraints
  - Variance (delta or bootstrap), with A via `build_el_jacobian(family, ...)` and B via `compute_score_variance_func` (IID vs survey), and analytic/numeric `∇g` inside `el_compute_delta_variance()` depending on trimming
...
- Scaling: `src_dev/shared/scaling.R` (class ‘nmar_scaling_recipe’, constructor, validator, create/apply/prepare, unscale).
- S3 Surface:
  - Parent `nmar_result` S3 methods in `src_dev/S3/nmar_result_methods.R` (`estimate`, `vcov`, `confint`, `tidy`, `glance`, `plot`, `autoplot`).
  - EL-specific S3 methods in `src_dev/engines/el/s3.R` (`print`/`summary` for `nmar_result_el`).
...
| $\hat{Y}$               | Estimator $\sum p_i^{\text{EL}} Y_i / \sum p_i^{\text{EL}}$ | estimate (field) / estimate(fit) |
| $F(\theta)$             | Estimating system                                      | el_build_equation_system       |
| $A$                     | $\partial F/\partial \theta$ (Jacobian)               | build_el_jacobian              |
| $B$                     | Covariance of scores                                   | compute_score_variance_func    |
| $g(\theta)$             | Functional for mean                                    | el_build_mean_fn               |
| $\text{Var}(\hat{Y})$   | Delta: $\nabla g \, A^{-1} B (A^{-1})^T \nabla g^T$    | el_compute_delta_variance      |
...
- Equations: `src_dev/engines/el/impl/equations.R`
- Jacobian: `src_dev/engines/el/impl/jacobian.R`
- Core solve + weights + variance: `src_dev/engines/el/impl/core.R`, `src_dev/engines/el/impl/variance.R`
- Scaling: `src_dev/shared/scaling.R`
