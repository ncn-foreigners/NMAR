...
- Specify the model with a two-sided formula: `Y_miss ~ X1 + X2` (auxiliaries are the RHS variables). If you have response‑only predictors that should enter the missingness model but not as auxiliaries, pass them via `response_predictors = c("Z1","Z2")`.
  - Variables on the outcome RHS (e.g., `X1 + X2`) are auxiliaries; supply their known population means via `auxiliary_means = c(X1 = ..., X2 = ...)`.
  - Predictors in `response_predictors` enter only the response model (no auxiliary constraint) and do not need population means.
- Choose the engine: `el_engine(...)`, e.g., `el_engine(auxiliary_means = c(X1 = 0), variance_method = "delta", standardize = TRUE)`.
- Fit: `nmar(formula = Y_miss ~ X1 + X2, data = df_or_design, engine = el_engine(...), response_predictors = c("Z1","Z2"))`.
...
- Solver method and Jacobian:
  - `solver_method = "auto"` (default) tries Newton (with the analytic Jacobian when available) and falls back to Broyden if needed. Force Broyden with `solver_method = "broyden"` for large‑p problems.
  - `solver_jacobian = "analytic"` uses the analytic Jacobian for solving; set `"none"` to force secant updates.
- Advanced nleqslv tuning:
  - Top‑level nleqslv args via `solver_args = list(global = "dbldog", xscalm = "auto")`.
  - Control‑list tolerances via `control = list(xtol = 1e-10, ftol = 1e-10, maxit = 200)`.
  - The effective settings used are recorded in `fit$diagnostics` (e.g., `solver_method`, `nleqslv_global`, `nleqslv_xscalm`).
...
- For practical scripts and experiments that informed this implementation, see the package’s development materials and the simulation script under `inst/simulations/mc-el.R`.
...
- `variance_method = "delta"` uses an analytic sandwich; it is very fast but may under‑estimate variability in tougher regimes (heavy nonresponse, ill‑conditioned Jacobians, or trimming). Prefer `variance_method = "bootstrap"` for more robust SEs.
