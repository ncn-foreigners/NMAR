...
You can include predictors that enter only the response model (and are not constrained as auxiliaries). Specify them via the `response_predictors` argument to `nmar(...)` while keeping the auxiliary set on the RHS of the outcome formula.
...
- Response-only predictors (in `response_predictors`) do not need auxiliary means.
...
- Response-only predictors: Variables in `response_predictors` do not need to appear on the RHS of the outcome formula; they enter only the response model. Auxiliary means must be supplied only for variables on the outcome RHS.
...
- Ill-conditioned Jacobian (large `fit$diagnostics$jacobian_condition_number`): delta variance may return NA by design; prefer `variance_method = "bootstrap"`. You may also tighten solver tolerances via `control = list(xtol=..., ftol=..., maxit=...)`.
