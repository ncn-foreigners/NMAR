...
# Requires the optional 'broom' package
  broom::tidy(fit)
  broom::glance(fit)
}
```
...
# Requires 'future.apply' (and its dependency 'future'); progressr is optional
...
- Inconsistent auxiliaries: If provided auxiliary means are grossly inconsistent with the sample, the engine will issue a warning via auxiliary inconsistency diagnostics and EL weights may go negative or the solver may fail. Consider revisiting the constraints, relaxing them, or using `trim_cap` and bootstrap variance.
