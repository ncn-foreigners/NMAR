...
[`nmar()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/nmar.md)
uses a two-sided formula with the outcome on the left-hand side. In the
common “missing values indicate nonresponse” workflow, nonrespondents
are encoded as `NA` in the outcome.

Many engines support a partitioned right-hand side via `|` (e.g.,
`y_miss ~ block1_vars | block2_vars`), but the interpretation of these
blocks is engine-specific. See
[`?el_engine`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/el_engine.md)
and
[`?exptilt_engine`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/exptilt_engine.md)
for details.
