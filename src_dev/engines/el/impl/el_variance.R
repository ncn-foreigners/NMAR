...
compute_score_contrib <- function(family,
  ...
  U_beta <- response_model_matrix_scaled * as.vector(
    (family$mu.eta(eta_i_hat) / p_hat) - lambda_W_hat * family$mu.eta(eta_i_hat) / denominator_hat
  )
  U_W <- (w_i_hat - W_hat) / denominator_hat
  U_aux <- if (K_aux > 0) sweep(auxiliary_matrix_scaled, 2, mu_x_scaled, "-") * as.vector(1 / denominator_hat) else matrix(nrow = nrow(response_model_matrix_scaled), ncol = 0)
...
build_mean_fn <- function(family,
  ...
  function(p) {
    ...
    sum(pi_final * outcome_vec) / sum(pi_final)
  }
}
