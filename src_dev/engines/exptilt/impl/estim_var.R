# temporary, so it doesent crash
#' @exportS3Method NULL
estim_var.nmar_exptilt <- function(model) {
# return(list(var_est = 0.69, vcov=0.69))
# return(list(var_est = NA, vcov = NA))
# return error
  stop("Variance estimation for nmar_exptilt is not yet implemented.")
}
...
#     sum_term <- t(u_i * model$design_weights / p_obs) %*% pi_deriv_obs
#
