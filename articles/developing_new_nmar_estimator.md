...
- Survey: convert to bootstrap replicate weights
  ...
  use
  [`survey::withReplicates`](https://rdrr.io/pkg/survey/man/withReplicates.html),
  then [`survey::svrVar`](https://rdrr.io/pkg/survey/man/svrVar.html).
...
- The parent S3 class `nmar_result` provides default methods
  (`estimate`, `vcov`, `confint`, `tidy`, `glance`, `plot`/`autoplot`,
  `coef`, `fitted`, `weights`, `formula`). Implement child print/summary
  only if you need extra presentation.
