...
- Construct results with
  ...
  and return class `nmar_result_<method>`.
...
3.  [`nmar()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/nmar.md)
    validates inputs and dispatches to
...
Call
[`new_nmar_result()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/new_nmar_result.md)
inside your constructor so the object carries class
`c("nmar_result_<method>", "nmar_result")` and shares the standard
layout:
...
so missing pieces are filled with safe defaults.
...
- `print.nmar_result_<method>()`, `summary.nmar_result_<method>()`
  ...
  [`NextMethod()`](https://rdrr.io/r/base/UseMethod.html) first to reuse
  the parent output and then append estimator-specific details.
- [`vcov()`](https://rdrr.io/r/stats/vcov.html),
  [`confint()`](https://rdrr.io/r/stats/confint.html), `tidy()`,
  `glance()`: these work out of the box as long as you populate the
  shared schema.
- Optional accessors:
  [`weights()`](https://rdrr.io/r/stats/weights.html),
  [`fitted()`](https://rdrr.io/r/stats/fitted.values.html),
  [`coef()`](https://rdrr.io/r/stats/coef.html), etc., if your engine
  exposes them.
...
- [`validate_and_apply_nmar_scaling()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/validate_and_apply_nmar_scaling.md)
  to standardize `Z`/`X` and auxiliary means before solving, returning
  an `nmar_scaling_recipe` for unscaling.
...
- [`logit_family()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/logit_family.md)
  ...
  expose `linkinv`, `mu.eta`, `d2mu.deta2`, and `score_eta` for
  link-agnostic estimating equations and Jacobians.
- Probit uses a tail-stable log-ratio for `phi/Phi`.

#### Bootstrap variance

- [`bootstrap_variance()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/bootstrap_variance.md)
  is an internal helper that handles `data.frame` and `survey.design`
  inputs (it is not a formal exported S3 generic).
...
- Survey: convert to bootstrap replicate weights
  ...
  then pass replicate estimates and the design’s `scale`/`rscales`/`mse`
  directly to
  [`survey::svrVar()`](https://rdrr.io/pkg/survey/man/svrVar.html).
- Engines may pass a `resample_guard(indices, data)` in the IID case to
  reject structurally invalid resamples (e.g., those with zero
  respondents).
- For survey designs, `survey_na_policy` controls how failed replicates
  are handled:
  - `"strict"` (default): any non-finite replicate estimate is an error.
  - `"omit"`: failed replicates are dropped and the corresponding
    `rscales` entries are removed before calling
    [`svrVar()`](https://rdrr.io/pkg/survey/man/svrVar.html).
...
- Prefix engine-specific helpers with `<method>_` (e.g., `el_...` for
  empirical likelihood) to signal ownership and avoid accidental reuse.
...
[`new_nmar_result()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/new_nmar_result.md)
(or a thin engine wrapper such as
[`new_nmar_result_el()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/new_nmar_result_el.md)).
The shared parent class has the following schema:
...
- Lists:
  ...
  - `weights_info`: list with
    - `values`: unnormalized analysis weights or masses on the engine’s
      scale,
    - `trimmed_fraction`: fraction of mass trimmed (numeric, `NA_real_`
      when not used).
  - `sample`: list with
  ...
  - `inference`: list with
    ...
    - `df`: degrees of freedom for t-based inference (numeric, may be
      `NA_real_`),
    - `message`: variance-related notes (character, may be
      `NA_character_`).
  - `diagnostics`: list for engine-specific diagnostics (Jacobian
    condition numbers, equation residuals, denominator summaries, and so
    on).
  - `meta`: list with
  ...
  - `extra`: list for any additional engine-specific objects (for
    example `nmar_scaling_recipe`, fitted probabilities, or cached raw
    structures).
...
- **Weights:** if you want
  [`weights()`](https://rdrr.io/r/stats/weights.html) to return analysis
  weights, store unnormalised masses in `weights_info$values` and the
  ...
  method then returns probability weights (`scale = "probability"`) and
  population weights (`scale = "population"`) that satisfy the
  documented sum relationships (up to numerical precision), even under
  trimming. Engines that use a different weighting concept can either
  map into this convention or provide an engine-specific
...
[`new_nmar_result()`](https://ncn-foreigners.ue.poznan.pl/NMAR/index.html/reference/new_nmar_result.md),
it automatically inherits a consistent interface for
[`vcov()`](https://rdrr.io/r/stats/vcov.html),
[`confint()`](https://rdrr.io/r/stats/confint.html), `tidy()`,
`glance()`, [`coef()`](https://rdrr.io/r/stats/coef.html),
[`fitted()`](https://rdrr.io/r/stats/fitted.values.html),
[`weights()`](https://rdrr.io/r/stats/weights.html),
